...
> Actualizado: 2026-09-08 · branch `main` · F85 DONE · Suite 2286/2286 · RC CLEAN · Audit Complete
...
| FASE 77 | DONE — Administrative Session Integrity: 3 leakage points corregidos — `_buildCheckInData` adherencia_pct usa `_isRealExecution` (SKIPPED ya no cuenta en adherencia), close-week banner distingue pendiente/completada/omitida con `_skipCnt`, coach monitor `doneCount` excluye `skipped:true` inline; tests F77-A–C (19 assertions) · Suite 2203/2203 |
| FASE 78 | DONE — Final Generation Quality Gate: `_runPreWritePlanGate(planObj, context)` unifica en una función determinista: structural (F70), load contract (numeric ≥ 0), per-day duplicate detection, VETO (cuando restrictions), nutrition macro coherence (Δ>3%), propagación de effGate/longGate sin reducir severidad. Status: PASS\|WARN\|REVIEW_REQUIRED\|BLOCK. `saveImportedPlan` y `_vdsenSaveDraftToFirestore` (generación) migrados. Tests F78-A–H (27 assertions) · Suite 2203/2203 |
| FASE 79 | DONE — Coach↔Client Parity Audit + Decision Trace UX: parity findings clean (technique normalization identical, RIR not duplicated, observationsCount filters autoFilled, deload by last-week per contract). Decision trace renderer upgraded: `_fmtTraceEntry()` shows type/decision/reason/source in distinct visual weight, falls back to raw JSON for unknown shapes. F78 gate added to generation save path (`_vdsenSaveDraftToFirestore`). Suite 2203/2203 |
| FASE 80 | DONE — Client UX Session State Display Parity: `isDone` now uses `_isRealExecution()` — SKIPPED sessions no longer show "✓ COMPLETA". New `_isSkipped` flag (explicit user skip only, not AUTO_CLOSED_NO_DATA). SKIPPED badge renders as "⏸ OMITIDA" with muted styling. Tests F80-A–F (11 assertions) · Suite 2214/2214 |
| FASE 81 | DONE — Performance: `loadPlan` exercise catalog + meso log reads parallelized with `Promise.all` — saves ~1 serial Firestore RTT (~200-400ms) on every client app load. No schema/behavior change; error paths unchanged. commit `2771363` · Suite 2214/2214 |
| RC AUDIT | DONE — Release Candidate Audit. HIGH fixed (F82). MEDIUM fixed (F83/F84). LOW fixed (F84/F85). HEAD `c27497e` is RC stable baseline. Suite 2286/2286 · Audit Complete |
| FASE 82 | DONE — `aggregateClientLogs.sessionsDone` inflated adherence by counting SKIPPED + AUTO_CLOSED_NO_DATA sessions. Fixed to count only REAL_COMPLETE + AUTO_CLOSED. commit `1af334b` · Suite 2222/2222 |
| FASE 83 | DONE — RC MEDIUM CLEANUP: (1) F78 gate integrated into `saveManualPlan` + `saveTrainingPlan` (vdsen-coach.html) — BLOCK aborts write with showToast, REVIEW_REQUIRED/WARN log to console.warn, same semantics as saveImportedPlan; (2) `_isSkipped` unified across all 3 cliente render surfaces to F80 formula (`done.skipped && !done.autoClosed`); day tabs (line 6474) use `_getSessionCompletionState()` — AUTO_CLOSED_NO_DATA now renders muted/⏸ as admin state, never as green/✓ or as user-SKIPPED; machine banner (line 4814) guard added. Tests F83-A–AB (28 assertions). commit `486dd2c` · Suite 2250/2250 |
| FASE 84 | DONE — Coach Monitor Session State Accuracy: 4 obsolete `done_` checks fixed in vdsen-coach.html. (1) `_coachCalcAdherence` `=== true` → showed 0% adherence for all modern sessions; (2) `_coachBuildBitacora` `=== true` → day header showed ○ for all modern sessions; (3) Mesocycle heatmap `!!entry` → AUTO_CLOSED_NO_DATA showed green ✓ instead of muted ⏸; (4) `_computeConfidenceScore doneDays` → SKIPPED sessions inflated density denominator. All four use canonical inline `v === true \|\| (typeof v === 'object' && v && !v.skipped)`. Tests F84-A–D (26 assertions). commit `3503b11` · Suite 2276/2276 |
| FASE 85 | DONE — Coach Alert mesoEnd false positive: `getClientAlert` truthy check on last-week `done_` entries caused "🏁 Mesociclo completado" banner when AUTO_CLOSED_NO_DATA (navigation-only, no real training) covered all final-week sessions. Fixed to exclude `autoClosed && skipped` entries; REAL_COMPLETE + AUTO_CLOSED + SKIPPED + legacy still count. Tests F85-A1–A10 (10 assertions). commit `c27497e` · Suite 2286/2286 |
| AUDIT COMPLETO | 2026-09-08 — Full autonomous audit of both apps (Coach + Cliente). CRITICAL: none found. HIGH: F82/F84 fixed. MEDIUM: F83/F84/F85 fixed. LOW remaining (intentional NO_ACTION): `_autoCloseAndAdvanceWeek` no-revert-on-failure (pre-existing, low probability), `_skipCnt` groups SKIPPED+AUTO_CLOSED_NO_DATA in close-week banner (display only). All canonical `_isRealExecution`/`_getSessionCompletionState` usage verified correct. Write gates on all 4 save paths confirmed. Generation pipeline clean. Progression engine correct. |
| FASE 76 | DONE — Session State Machine: `_getSessionCompletionState` / `_isRealExecution` helpers; 5 estados canónicos (REAL_COMPLETE, SKIPPED, AUTO_CLOSED, AUTO_CLOSED_NO_DATA, PENDING); `skipSession()` en vdsen-cliente.html; icon ⏸ en tabs/monitor; `countDone()` y streak usan `_isRealExecution`; tests F76-A–I (34 assertions) · Suite 2157/2157 |
| FASE 75 | DONE — Contextual Auto Cardinality Optimizer: `_extractTDSignals` + `_scoreTrainingDaysCandidate` + `_extractMPDSignals` + `_scoreMealsPerDayCandidate` puros; `_resolveTrainingDaysIntent` AUTO reemplazado por evaluador determinista multi-señal: evalúa candidatos 2..min(diasFicha||7,7), señales: NIVEL (score, no regla), SESSION_DURATION (<45min→más días, >75min→menos), PRIORITY_GROUPS (od=ceil(gCount/2)), READINESS_FATIGUED (cuadrática: pico d=3, empate d=3/d=4→tiebreak min-complejidad), READINESS_PROGRESSING (lineal positiva), ARTICULAR_ISSUES, PREV_PLAN_CONTINUITY, OBJETIVO (+0.5 volume≥4/cut≤4), DELOAD_CANDIDATE; CLIENT_AVAILABILITY_LIMIT cuando winner no-restringido > diasFicha; retorna {resolvedValue, source:'ENGINE_OPTIMIZED'/'CLIENT_AVAILABILITY_LIMIT', reasonCodes, confidence, candidateScores}; `_resolveMealsPerDayIntent` AUTO con scorer análogo: HIGH_CALORIE_TARGET (0.4-|m-5|*0.4), LOW_CALORIE_TARGET (0.4-|m-3|*0.4), HIGH_PROTEIN (m≥4:+0.3), LOW_PROTEIN (m≤4:+0.2), HORARIO_RESTRINGIDO (cuadrática baja), OBJETIVO_VOLUMEN (0.4-|m-5|*0.4), OBJETIVO_DEFINICION (0.4-|m-4|*0.4), COMPLEXITY_PENALTY (-(m-3)*0.15); SAFE_DEFAULT=4 cuando _hasSig=false; guards F74 intactos; tests F75-A–P (37 assertions nuevos) · Suite 2123/2123 |
| FASE 74 | DONE — Cardinality Conformance + Policy Correction: `_resolveTrainingDaysIntent` AUTO reescrito — `nivelOptimal` calculado primero (p=3, i=4, a=5), `diasFicha` es techo no objetivo; si `diasFicha < nivelOptimal` → CLIENT_AVAILABILITY_LIMIT con `resolvedValue=diasFicha`; si `diasFicha >= nivelOptimal` → ENGINE_OPTIMIZED/SAFE_DEFAULT con `resolvedValue=nivelOptimal`; `diasFicha=7, nivel=avanzado` → 5 (no 7 ni 6); conflicto EXPLICIT > disponibilidad cambiado de SOFT a REVIEW_REQUIRED; `_guardCardinalityConformance(resolvedDays, resolvedMeals, training, nutrition)` puro devuelve `{valid, errors[]}` validando `daysPerWeek===resolvedDays`, `days.length===resolvedDays`, `comidas.length===resolvedMeals`; guard pre-call `_askConfirm` en ambos flujos cuando hay conflicto REVIEW_REQUIRED (bloquea API hasta confirmación del coach); guard pre-write `_guardCardinalityConformance` antes de `saveImportedPlan` en ambos flujos (throw en divergencia); tests F73-B/C/D/G actualizados para nueva lógica; bloque F74 nuevo (28 assertions); Suite 2111/2111 |
| FASE 73 | DONE — Intent Resolution Engine: `_resolveTrainingDaysIntent(intentSlot, fichaData, prescCtx, longitudinalCtx)` y `_resolveMealsPerDayIntent(intentSlot, fichaData, nutritionCtx)` puros (0 I/O, 0 writes, 0 mutation); devuelven `{requestedMode, requestedValue, resolvedValue, source, reasonCodes, confidence, conflicts}`; sources: COACH_EXPLICIT | CLIENT_AVAILABILITY_LIMIT | CLIENT_PREFERENCE | ENGINE_OPTIMIZED | SAFE_DEFAULT; prioridades días: (1) ficha.dias_semana clamped a 6, (2) nivel→3/4/5, (3) SAFE_DEFAULT=4; prioridades comidas: (1) ficha.num_comidas clamped a [3,6], (2) kcal>3000→5/kcal≤1800→3, (3) objetivo volumen→5/definición→4, (4) SAFE_DEFAULT=4; EXPLICIT conserva exactamente el valor del coach salvo conflict SOFT (>disponibilidad ficha); `_buildResolvedIntentConstraintText(tdR, mpR)` genera CARDINALIDAD FIJA siempre con valor específico (nunca "decide tú"); ambos flujos (autoGeneratePlan + _autoGenerateForModal) usan `_resolvedConstraintTxt`/`_resolvedConstraintTxt2` en lugar del viejo `_intentConstraintTxt`; preview CONFIGURACIÓN RESUELTA usa valores pre-resueltos con fuente expandida (📋 Ficha/Preferencia, ⚙️ VDSEN/Default, 👤 Coach) y muestra conflict warning si existe; LLM recibe siempre números concretos, nunca elige días/comidas libremente; tests F73-A–Q (68 assertions) · Suite 2082/2082 |
| FASE 72 | DONE — Learned State Preview Block: IIFE `_lsHintHtml` / `_lsHintHtml2` renderiza resumen compacto del learned state en preview de ambos flujos de generación (autoGeneratePlan + _autoGenerateForModal); bloque teal "🧠 HISTORIAL ACTIVADO" aparece entre CONFIGURACIÓN RESUELTA y AUDITORÍA; muestra hasta 3 líneas de datos (excluye línea header); líneas truncadas a 100 chars; HTML-escaped para prevenir XSS; vacío cuando `_lsHint` es '' (no hay historial); no bloquea write; tests F72-A–G (16 assertions) · Suite 2014/2014 |
| FASE 71 | DONE — Learned State Prompt Hint: `_buildLearnedStatePromptHint(activeLearnedState)` puro (0 I/O, 0 writes, 0 mutation) extrae resumen accionable del learned state para inyectar en el prompt del motor API; incluye: TOPOLOGÍAS PREFERIDAS/RECHAZADAS (topologyState.preferredPatterns/rejectedPatterns), EJERCICIOS CON HISTORIAL POSITIVO (confidence≥medium, continuityType KEPT/MOVED, observaciones positivas), EJERCICIOS CON SEÑAL DE DOLOR (painSignals.length>0); excluye: confidence none/low, pain sin mark positivo; devuelve '' cuando no hay datos accionables (sin encabezado LEARNED STATE); integrado en ambos flujos autoGeneratePlan + _autoGenerateForModal como `_lsHint`/`_lsHint2` inyectado en userMsg entre _intentConstraintTxt y _missingDataTxt; tests F71-A–J (19 assertions) · Suite 1995/1995 |
| FASE 70 | DONE — Pre-Write Plan Integrity Guard: `_guardPlanIntegrity(planObj)` puro (0 I/O, 0 writes, 0 mutation) valida integridad estructural antes de escribir en Firestore; returns `{valid, errors[], warnings[]}`; `valid:false` → caller aborta write (días vacíos, día sin ejercicios, ejercicio sin sets); warnings (no bloquean): exerciseName vacío, repsTarget<1, rirTarget<0, restSeconds≤0, campos FORBIDDEN presentes; RIR=0 explícitamente válido; integrado en `saveImportedPlan` — `showToast` de error + `return` si `!valid`, `console.warn` de warnings si `valid`; tests F70-A–K (21 assertions) · Suite 1977/1977 |
| FASE 69 | DONE — Ficha Completeness Indicator: `_classifyMissingInputs(fichaData, {})` llamado en `_renderClientTabFicha` usando `_detailFichaData` cacheado (0 Firestore reads); indicador compacto inyectado en tarjeta Motor VDSEN justo debajo del título: ✅ "Ficha completa para generación" cuando no hay RECOMMENDED ausentes, ⚡ "N campo(s) recomendado(s) ausente(s) — el motor usará defaults" + lista de field names cuando sí hay; visible antes de hacer clic en GENERAR para que el Coach conozca calidad de datos; sin bloqueo — `blockingCount` siempre 0; integración: 2 str_replace quirúrgicos en `_renderClientTabFicha`; tests F69-A–E (11 assertions) · Suite 1956/1956 |
| FASE 68 | DONE — Plan Quality Audit: `_buildPlanQualityAudit(training, nutrition, supplements)` puro (0 I/O, 0 writes, post-generation read-only) produce auditoría de calidad compacta; status: OK/WARNINGS/REVIEW; stats: days/exercises/sets/meals; issues: WARN (calidad) / REVIEW (bloqueante): restSeconds≤0, repsTarget inválido, rirTarget<0, exerciseName vacío, sin comidas, calorías<100, macros inválidos, tiers vacíos; RIR=0 explícitamente válido (no se reporta como WARN); bloque "AUDITORÍA: STATUS" con color semáforo y conteo en preview de ambos flujos (post-write, nunca bloquea); tests F68-A–K (24 assertions) · Suite 1946/1946 |
| FASE 67 | DONE — Missing Data Classification: `_classifyMissingInputs(fichaData, prescCtx)` puro (0 I/O, 0 writes, 0 mutation) clasifica campos ausentes en la ficha y datos longitudinales antes de la generación; severity RECOMMENDED (impacto en calidad) / INFO (sin impacto); blockingCount siempre 0 (nunca REQUIRED — el motor maneja todos los defaults); RECOMMENDED items inyectados en el prompt API como "NOTA — Datos ausentes que el motor debe estimar con defaults" (sin restricción, solo contexto); bloque "⚠ DATOS AUSENTES (N)" en preview amarillo cuando hay RECOMMENDED items en ambos flujos (autoGeneratePlan + _autoGenerateForModal); vdsen-plan-v2 intacto; tests F67-A–J (30 assertions) · Suite 1922/1922 |
