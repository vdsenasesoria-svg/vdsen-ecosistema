...
> Actualizado: 2026-09-08 · branch `main` · RC AUDIT F82 DONE · Suite 2222/2222
...
| RC AUDIT | DONE — Release Candidate Audit. HIGH finding fixed (F82): `aggregateClientLogs.sessionsDone` inflated adherence by counting SKIPPED + AUTO_CLOSED_NO_DATA sessions. Fixed to count only REAL_COMPLETE + AUTO_CLOSED. MEDIUM findings documented as backlog (F78 gate gap in `saveManualPlan`/`saveTrainingPlan`; `_isSkipped` formula inconsistency between render paths at lines 4814/6475 vs 6543). LOW: `_computeConfidenceScore` doneDays over-counts. commit `1af334b` · Suite 2222/2222 |
