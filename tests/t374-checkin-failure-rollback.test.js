...
test('failed check-in save restores the prior persisted entry', async () => {
  global._guardarCIInFlight = false;
  global.CURRENT_WEEK = 2;
  global.LOGS = { ci_sem_2: { peso: '70', nota: 'previa' } };
  global.LOGS_BY_WEEK = { log: { 2: [] } };
  global._saveLogsTimer = null;
  global.USER = { uid: 'client-a', email: '' };
  global.FB = null;
  global.window = { _ciSubjTemp: { ci_animo: 4 } };
  ...
  await guardarCI();
  assert.deepEqual(LOGS.ci_sem_2, { peso: '70', nota: 'previa' });
